...
// @input vec3[] positionRotation {"label": "Rotation"}
//@ui {"widget":"separator"}
        ...
        for (var j = 0; j < duplicatedObjects.length; j++) {
            ...
            if (!isNull(duplicatedObjects[j])) {
                ...
                if (!disableRotation) {
                    ...
                    var axis = script.positionRotation[j];
                    var rotationToApply = quat.angleAxis(radians, axis);
                    duplicatedObjects[j].getTransform().setWorldRotation(rotationToApply);
                }
            }
        }
    }
...
global.createObjects = function(count) {
